...
from src.adapter.imap.provider_presets import preset_for_email, slug_for_email
from src.application.dto.display_settings_dto import DisplaySettingsDTO
...
class MailService:
    ...
    def add_account(self, data: dict[str, Any]) -> None:
        ...
        account = MailAccountDTO(
            ...
            email=email,
            username=data["username"].strip() or email,
    ...
    def cached(self, folder: str = "inbox") -> dict[str, Any]:
        ...
        if folder == "starred":
            ...
            for m in self._store.list_all():
                if self._addr(m.sender) in blocked and (m.account, m.folder, m.uid) not in seen:
                    source.append(m)
        else:
            source = [m for m in self._store.list(folder) if self._addr(m.sender) not in blocked]
    ...
    def poll_new(self) -> list[dict[str, Any]]:
        ...
        new = [
            ...
            and self._addr(m.sender) not in blocked
        ]
    ...
    def _to_dicts(self, rows: list[MessageDTO]) -> list[dict[str, Any]]:
        ...
        for m in rows:
            ...
            data["seen"] = reads.get(key, m.seen)
            data["spam_sender"] = self._addr(m.sender) in blocked
            result.append(data)
        return result
