...
class ProviderPreset:
    ...
    imap_port: int
    smtp_host: str
    smtp_port: int  # 465 -> implicit SSL, otherwise STARTTLS
    note: str = ""
    auth: str = "password"  # "password" or "oauth2"
    oauth_provider: str | None = None  # slug into OAUTH_PROVIDERS when auth=="oauth2"


# Keyed by a short provider slug. This is the whole point of the aggregator:
# add a service here once and every account of that kind is configured for free.
...
# Map an email domain to the provider slug above.
...
def preset_for_email(email: str) -> ProviderPreset | None:
    """
    Guess the provider from an email address' domain.

    Returns None for unknown domains (self-hosted, corporate) — the caller then
    asks for hosts/ports manually.
    """
    _, _, domain = email.partition("@")
